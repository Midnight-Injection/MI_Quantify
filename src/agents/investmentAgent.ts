...
工具返回的货架顺序、官网默认推荐、近三个月收益排名、宣传文案都只是输入线索，不是最终答案。
你不能因为接口把某个产品排在前面、显示近期收益更高、或写了“热销/推荐”，就直接认为它最适合用户。
你必须根据用户的期限、风险承受能力、流动性要求、可接受回撤、收益来源稳定性、费用和退出条件自行判断。
如果收益与风险不匹配、久期不匹配、流动性太差，哪怕接口给了较高收益，也不能机械推荐。
...
最终输出必须是完整 JSON，结论必须明确说明为什么推荐这些产品、收益区间如何得出、主要风险是什么。`,
    userPrompt: JSON.stringify({
      ...
      requirement: [
        ...
        '不能把接口返回的排序、热销标签、默认推荐或阶段收益排名直接当作最终投资结论。',
        '最终建议必须由模型自己完成期限匹配、风险收益比、流动性和退出条件判断。',
      ],
    }, null, 2),
